...
export default function Dashboard() {
  ...
  const handleSearch = async (ticker: string, p1: number, p2: number, entry: number, reinvest: number) => {
    setLoading(true)
    try {
      const json = await fetchHistoricalData(ticker, p1, p2, entry, reinvest)
