...
export default function Dashboard() {
  ...
  const handleSearch = async (ticker: string, p1: number, p2: number) => {
    setLoading(true);
    try {
      const json = await fetchHistoricalData(ticker, p1, p2);
      setChartData(json);
    } catch (error) {
  ...
  return (
    <main>
      ...
      {loading ? <p>Loading...</p> : <ChartLineInteractive data={chartData} />}
    </main>
  );
}